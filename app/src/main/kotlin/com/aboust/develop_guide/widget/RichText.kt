 ...
 *
 */
class RichText(private val context: Context, text: CharSequence) : SpannableString(text) {
    ...
    fun all(target: String) = apply {
        rangeList.clear()
        val indexes = offsetIndex(toString(), target)
     ...
     */
    fun offsetIndex(src: String, target: String): MutableList<Int> {
    ...
    public fun RichText.applySpan(span: Any) = apply {
    ...
    /**
     * 字体颜色 [hex]表示target字串的字体颜色
     */
    fun textColor(hex: String) = applySpan(ForegroundColorSpan(Color.parseColor(hex)))
